...
const
  ...
  FILE_FFMPEG_DOWNLOADED = FILE_DIR + '\' + FILE_FFMPEG_DOWNLOADED_INDIR + '.zip';
  ...
  LATEST_YTDLP_DOWNLOAD_URL = 'https://github.com/yt-dlp/yt-dlp/releases/latest/download/yt-dlp.exe';
  ...
  LATEST_TTS_DOWNLOAD_URL = 'https://rhvoice.su/downloads/?voice=aleksandr-hq&type=sapi&version=4.1.2011.17';
  BIG_UI_MUL = 1.25;
  MAX_INSTANCES = 32;
  BASE_MUTEX_NAME = 'DYDownFMX_' + APP_VER + '__Instance_';
  videoResolutions: TArray<Integer> = [144, 240, 360, 480, 720, 1080, 1440, 2160];
  audioBitrates: TArray<Integer> = [128, 192, 320];
  ERROR_URL_NUMBER_REGEX: string = '^#URL::(\d+)::';
  DEFAULT_LOCALE = 'ru';
  DEFAULT_COOKIES_FILE = FILE_DIR + '\cookies.txt';
  bruhQueries: TArray<string> = ['.рф'];
